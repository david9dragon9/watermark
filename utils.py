...
def get_target_modules(
    model: nn.Module, custom_phrase: Optional[str] = None
) -> List[str]:
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, linear_classes) and (
            custom_phrase is None or custom_phrase in name
        ):
...
def select_last_token_hidden_state(
    model: nn.Module,
...
def setup_wandb(args):
    wandb_group = (
        ...
        else args.root_dir.split("/")[-1]
    )
    wandb_name = args.wandb_name if args.wandb_name is not None else wandb_group
    wandb.init(
        project="langshift", group=wandb_group, name=wandb_name, config=args.__dict__
    )
...
class PairedDataloader:
    def __init__(self, dataset1, dataset2, batch_size1, batch_size2, shuffle=True):
        ...
        self.length1, self.length2 = math.ceil(
            len(self.dataset1) / self.batch_size1
        ), math.ceil(len(self.dataset2) / self.batch_size2)
    ...
    def __getitem__(self, idx):
        ...
        collated1, collated2 = torch.utils.data.default_collate(
            batch1
        ), torch.utils.data.default_collate(batch2)
