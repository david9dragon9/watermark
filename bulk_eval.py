...
from langshift.common import initialize
from arguments import Arguments
from utils import setup_wandb, seed
    ...
    for step, batch in enumerate(pbar):
        with accelerator.autocast():
            ...
            if len(inputs) == 2:
                ...
                inputs_chosen = [
                    prompt + response
                    for prompt, response in zip(prompts, responses_chosen)
                ]
                inputs_rejected = [
                    prompt + response
                    for prompt, response in zip(prompts, responses_rejected)
                ]
                ...
                outputs = torch.cat([outputs_chosen, outputs_rejected], dim=1).reshape(
                    -1, 1
                )
                embedding_dim = embeddings_chosen.shape[-1]
                embeddings = torch.stack(
                    [embeddings_chosen, embeddings_rejected], dim=1
...
def main():
    ...
    model, optimizer, dataloader, loss_fn, eval_dataloader, eval_loss_fn = initialize(
        args, accelerator
    )

    all_outputs, all_embeddings = bulk_eval(model, eval_dataloader, args, accelerator)
    output_path, embedding_path = (
        args.bulk_eval_output_path,
        args.bulk_eval_embedding_path,
    )
