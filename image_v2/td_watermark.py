...
def watermark_image(
    ...
    if mode in ["watermark", "both"]:
        ...
        for step in pbar:
            ...
            if p_value < 0.01 and model.param.abs().max() < 0.01:
                break
            if p_value < 0.01:
...
def main():
    ...
    if args.mode == "both":
        output_pil = watermark_image(input_pil, None, args.password, "watermark")
        watermark_image(input_pil, output_pil, args.password, "detect")
    elif args.mode == "watermark":
        watermark_image(input_pil, None, args.password, args.mode)
    elif args.mode == "detect":
        output_pil = Image.open(args.output_path)
        watermark_image(None, output_pil, args.password, args.mode)
