...
def get_llm(
    ...
    # llm.get_tokenizer().add_bos_token = False
    return llm
...
def batch_inference(
    ...
    if apply_prompt_processor:
        ...
        prompts = [
            prompt_processor.process(description) for description in descriptions
        ]
    else:
    ...
    if llm is None:
        llm = get_llm(
            model_name if (model_path is None or lora) else model_path,
            lora=lora,
            gpu_memory_utilization=gpu_memory_utilization,
        )
...
def main():
    ...
    llm = get_llm(
        args.model_name if (args.model_path is None or args.lora) else args.model_path,
        lora=args.lora,
